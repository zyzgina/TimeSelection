package com.github.timeselection.view;
...
import com.github.timeselection.R;
import com.github.timeselection.util.TextUtil;
import com.github.timeselection.util.TimeScreenUtil;
import com.github.timeselection.util.TimeUtils;
...
public class TimeSelector {
    ...
    /**
     * 注释：使用该方法时，显示必须用show(Textview view)；
     *
     * @param context
     * @param startDate 开始时间 (注释:填年份)
     * @param endDate   结束时间
     */
    public TimeSelector(Context context, String startDate, String endDate) {
        this.context = context;
        this.handler = null;
        startCalendar = Calendar.getInstance();
        endCalendar = Calendar.getInstance();
        startDate = TimeUtils.formatYear(startDate, FORMAT_STR) + "-01-01 00:00";
        startCalendar.setTime(TimeUtils.parse(startDate, FORMAT_STR));
        endCalendar.setTime(TimeUtils.parse(endDate, FORMAT_STR));
        initDialog();
        initView();
    }

    /**
     * 注释：使用该方法时，显示用show()即可，否则显示的时间将会是你TextView上所携带的时间
     *
     * @param context
     * @param startDate    开始时间(注释:填年)
     * @param endDate      结束时间
     * @param selectorDate 选中时间
     */
    public TimeSelector(Context context, ResultHandler resultHandler, String startDate, String endDate, String selectorDate) {
        this.context = context;
        ...
        curCalendar.setTime(TimeUtils.parse(selectorDate, FORMAT_STR));
        initDialog();
        initView();
    }
    ...
    public TimeSelector(Context context, ResultHandler resultHandler, String startDate, String endDate) {
        ...
        endCalendar.setTime(TimeUtils.parse(endDate, FORMAT_STR));
        curCalendar = Calendar.getInstance();
        curCalendar.setTime(TimeUtils.parse(startDate, FORMAT_STR));
        initDialog();
        initView();
    }
    ...
    public void show(final TextView tvDate) {
        ...
        this.tvDate = tvDate;
        curCalendar = Calendar.getInstance();
        curCalendar.setTime(TimeUtils.parse(tvDate.getText().toString(), FORMAT_STR));
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                tvDate.setText(TimeUtils.format(selectedCalender.getTime(), FORMAT_STR));
    ...
    public void show() {
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                handler.handle(TimeUtils.format(selectedCalender.getTime(), FORMAT_STR));
                seletorDialog.dismiss();
            }
        });
        seletorDialog.show();
    ...
    private void initDialog() {
        if (seletorDialog == null) {
            ...
            int width = TimeScreenUtil.getInstance(context).getScreenWidth();
            lp.width = width;
    ...
    private void setCurDate() {
        ...
        if (curMininute == 0) {
            ...
            minute_pv.setSelected(curMininute - 1);
        }
    }
